...
/* パーサ */
struct WAVParser {
    MemoryFile *fp;             /* 読み込みファイルポインタ */
    struct WAVBitBuffer buffer; /* ビットバッファ */
};

/* ライタ */
struct WAVWriter {
    MemoryFile *fp;             /* 書き込みファイルポインタ */
...
/* パーサの初期化 */
static void WAVParser_Initialize(struct WAVParser *parser, MemoryFile *fp);
/* パーサの使用終了 */
static void WAVParser_Finalize(struct WAVParser *parser);
/* n_bit 取得し、結果を右詰めする */
static WAVError WAVParser_GetBits(struct WAVParser *parser, uint32_t n_bits,
                                  uint64_t *bitsbuf);
/* シーク（fseek準拠） */
static WAVError WAVParser_Seek(struct WAVParser *parser, int32_t offset,
                               int32_t wherefrom);
/* ライタの初期化 */
static void WAVWriter_Initialize(struct WAVWriter *writer, MemoryFile *fp);
/* ライタの終了 */
static void WAVWriter_Finalize(struct WAVWriter *writer);
/* valの下位n_bitを書き込む */
static WAVError WAVWriter_PutBits(struct WAVWriter *writer, uint64_t val,
                                  uint32_t n_bits);
/* バッファにたまったビットをクリア */
static WAVError WAVWriter_Flush(struct WAVWriter *writer);
/* リトルエンディアンでビットパターンを出力 */
static WAVError WAVWriter_PutLittleEndianBytes(struct WAVWriter *writer,
                                               uint32_t nbytes, uint64_t data);

/* ライタを使用してファイルフォーマットに従ったヘッダ部を出力 */
static WAVError WAVWriter_PutWAVHeader(struct WAVWriter *writer,
                                       const struct WAVFileFormat *format);
/* ライタを使用してPCMデータ出力 */
static WAVError WAVWriter_PutWAVPcmData(struct WAVWriter *writer,
                                        const struct WAVFile *wavfile);

/* リトルエンディアンでビットパターンを取得 */
static WAVError WAVParser_GetLittleEndianBytes(struct WAVParser *parser,
                                               uint32_t nbytes,
                                               uint64_t *bitsbuf);
/* パーサを使用して文字列取得 */
static WAVError WAVParser_GetString(struct WAVParser *parser,
                                    char *string_buffer,
                                    uint32_t string_length);
/* パーサを使用して文字列取得/一致チェック */
static WAVError WAVParser_CheckSignatureString(struct WAVParser *parser,
                                               const char *signature,
                                               uint32_t signature_length);
/* パーサを使用してファイルフォーマットを読み取り */
static WAVError WAVParser_GetWAVFormat(struct WAVParser *parser,
                                       struct WAVFileFormat *format);
/* パーサを使用してPCMデータを読み取り */
static WAVError WAVParser_GetWAVPcmData(struct WAVParser *parser,
...
static WAVError WAVParser_GetWAVFormat(struct WAVParser *parser,
    ...
    if (WAVParser_GetLittleEndianBytes(parser, 2, &bitsbuf) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    }
    if (bitsbuf != 1) {
    ...
    if (fmt_chunk_size > 16) {
        ...
        if (WAVParser_Seek(parser, fmt_chunk_size - 16, SEEK_CUR) !=
            WAV_ERROR_OK) {
            return WAV_ERROR_IO;
        }
    }
    ...
    while (1) {
        ...
        if (strncmp(string_buf, "data", 4) == 0) {
            ...
            if (WAVParser_GetLittleEndianBytes(parser, 4, &bitsbuf) !=
                WAV_ERROR_OK) {
                return WAV_ERROR_IO;
            }
    ...
    assert(tmp_format.num_samples %
               ((tmp_format.bits_per_sample / 8) * tmp_format.num_channels) ==
           0);
    tmp_format.num_samples /=
        ((tmp_format.bits_per_sample / 8) * tmp_format.num_channels);
...
static WAVError WAVParser_GetWAVPcmData(struct WAVParser *parser,
    ...
    for (sample = 0; sample < wavfile->format.num_samples; sample++) {
        for (ch = 0; ch < wavfile->format.num_channels; ch++) {
            if (WAVParser_GetLittleEndianBytes(parser, bytes_per_sample,
                                               &bitsbuf) != WAV_ERROR_OK) {
                return WAV_ERROR_IO;
            }
            /* 32bit整数形式に変形してデータにセット */
            wavfile->data[ch][sample] =
                convert_to_sint32_func((int32_t)(bitsbuf));
        }
    }
...
WAVApiResult WAV_GetWAVFormatFromFile(const void *buffer, size_t size,
                                      struct WAVFileFormat *format) {
...
struct WAVFile *WAV_CreateFromFile(void *buffer, size_t size) {
    struct WAVParser parser;
    MemoryFile *fp;
    struct WAVFile *wavfile;
    struct WAVFileFormat format;
...
struct WAVFile *WAV_Create(const struct WAVFileFormat *format) {
    ...
    /* データ領域の割り当て */
    wavfile->data =
        (WAVPcmData **)malloc(sizeof(WAVPcmData *) * format->num_channels);
    if (wavfile->data == NULL) {
        goto EXIT_FAILURE_WITH_DATA_RELEASE;
    }
    for (ch = 0; ch < format->num_channels; ch++) {
        wavfile->data[ch] =
            (WAVPcmData *)calloc(format->num_samples, sizeof(WAVPcmData));
...
static WAVError WAVParser_GetBits(struct WAVParser *parser, uint32_t n_bits,
    ...
    /* 初回読み込み */
    if (buf->byte_pos == -1) {
        if (memfread(buf->bytes, sizeof(uint8_t), WAVBITBUFFER_BUFFER_SIZE,
                     parser->fp) == 0) {
            return WAV_ERROR_IO;
        }
        buf->byte_pos = 0;
    ...
    while (n_bits > buf->bit_count) {
        ...
        tmp |= (uint64_t)WAV_GetLowerBits(buf->bit_count,
                                          buf->bytes[buf->byte_pos])
               << n_bits;
    ...
    tmp |= (uint64_t)WAV_GetLowerBits(
        n_bits, (uint32_t)(buf->bytes[buf->byte_pos] >> buf->bit_count));
...
static WAVError WAVParser_Seek(struct WAVParser *parser, int32_t offset,
    ...
    /* 移動 */
    memfseek(parser->fp, offset, wherefrom);
    /* バッファをクリア */
    parser->buffer.byte_pos = -1;
...
static WAVError WAVWriter_PutWAVHeader(struct WAVWriter *writer,
    ...
    /* PCM データサイズ */
    pcm_data_size = format->num_samples * (format->bits_per_sample / 8) *
                    format->num_channels;

    /* ファイルサイズ */
    filesize = pcm_data_size +
               44; /* "RIFF" から ("data"のサイズ) までのフィールドのバイト数
                      拡張部分を一切含まない */

    /* ヘッダ 'R', 'I', 'F', 'F' を出力 */
    if (WAVWriter_PutBits(writer, 'R', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, 'I', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, 'F', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, 'F', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    ...
    if (WAVWriter_PutLittleEndianBytes(writer, 4, filesize - 8) !=
        WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    }

    /* ヘッダ 'W', 'A', 'V', 'E' を出力 */
    if (WAVWriter_PutBits(writer, 'W', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, 'A', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, 'V', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, 'E', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    ...
    if (WAVWriter_PutBits(writer, 'm', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, 't', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, ' ', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };

    /* fmtチャンクのバイト数を出力 （補足）現在は16byte決め打ち */
    if (WAVWriter_PutLittleEndianBytes(writer, 4, 16) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };

    /* フォーマットIDを出力 （補足）現在は1（リニアPCM）決め打ち */
    if (WAVWriter_PutLittleEndianBytes(writer, 2, 1) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };

    /* チャンネル数 */
    if (WAVWriter_PutLittleEndianBytes(writer, 2, format->num_channels) !=
        WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };

    /* サンプリングレート */
    if (WAVWriter_PutLittleEndianBytes(writer, 4, format->sampling_rate) !=
        WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };

    /* データ速度（byte/sec） */
    if (WAVWriter_PutLittleEndianBytes(
            writer, 4,
            format->sampling_rate * (format->bits_per_sample / 8) *
                format->num_channels) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    }

    /* ブロックあたりサイズ数 */
    if (WAVWriter_PutLittleEndianBytes(
            writer, 2, (format->bits_per_sample / 8) * format->num_channels) !=
        WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    }

    /* 量子化ビット数（サンプルあたりのビット数） */
    if (WAVWriter_PutLittleEndianBytes(writer, 2, format->bits_per_sample) !=
        WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };

    /* "data" チャンクのヘッダ出力 */
    if (WAVWriter_PutBits(writer, 'd', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    };
    if (WAVWriter_PutBits(writer, 'a', 8) != WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    ...
    /* 波形データバイト数 */
    if (WAVWriter_PutLittleEndianBytes(writer, 4, pcm_data_size) !=
        WAV_ERROR_OK) {
        return WAV_ERROR_IO;
    }
...
static WAVError WAVWriter_PutWAVPcmData(struct WAVWriter *writer,
    ...
    for (sample = 0; sample < wavfile->format.num_samples; sample++) {
        for (ch = 0; ch < wavfile->format.num_channels; ch++) {
            if (WAVWriter_PutLittleEndianBytes(
                    writer, bytes_per_sample,
                    (uint64_t)convert_sint32_to_pcmdata_func(
                        WAVFile_PCM(wavfile, sample, ch))) != WAV_ERROR_OK) {
                return WAV_ERROR_IO;
            }
        }
    }
...
/* ファイル書き出し */
WAVApiResult WAV_WriteToFile(void *buffer, size_t size,
                             const struct WAVFile *wavfile) {
    struct WAVWriter writer;
    MemoryFile *fp;

    /* 引数チェック */
    if (buffer == NULL || wavfile == NULL) {
...
static WAVError WAVWriter_PutBits(struct WAVWriter *writer, uint64_t val,
    ...
    while (n_bits >= writer->bit_count) {
        ...
        writer->bit_buffer |=
            (uint8_t)WAV_GetLowerBits(writer->bit_count, val >> n_bits);

        /* バッファに追記 */
        writer->buffer.bytes[writer->buffer.byte_pos++] =
            (uint8_t)(writer->bit_buffer & 0xFF);

        /* バッファが一杯になったら書き出し */
        if (writer->buffer.byte_pos == WAVBITBUFFER_BUFFER_SIZE) {
            if (memfwrite(writer->buffer.bytes, sizeof(uint8_t),
                          WAVBITBUFFER_BUFFER_SIZE,
    ...
    writer->bit_buffer |=
        (uint8_t)(WAV_GetLowerBits(n_bits, (uint32_t)val) << writer->bit_count);
...
static WAVError WAVWriter_Flush(struct WAVWriter *writer) {
    ...
    if (writer->bit_count != 8) {
        if (WAVWriter_PutBits(writer, 0, (uint8_t)writer->bit_count) !=
            WAV_ERROR_OK) {
            return WAV_ERROR_IO;
        }
        writer->bit_buffer = 0;
    ...
    /* バッファに残っているデータをフラッシュ */
    if (memfwrite(writer->buffer.bytes, sizeof(uint8_t),
                  (uint32_t)writer->buffer.byte_pos,
