    ...
    *Qss = 10;
    ...
    FOR ( i=0; i<num_subband_smooth_fx; i++ )
    {
        ...
        FOR( k = 0; k < L_SB; k++ )
        {
            ...
            ELSE IF ( L_sub(L_abs(L_inBuf_pss[j]), L_max_val[i] ) < 0x0L )
            {
                IF( L_inBuf_pss[j] >= 0 )
                {
...
)
{
    ...
    up_lmt   = 0;
    move16();
    bits_req = bits_lagIndices_modeNormal_fx;
    move16();
    bits = 0;
    move16();
...
)
{
    ...
    {
        test();
        IF( sub(inner_frame_fx, length_fx) == 0 || bws_cnt_fx > 0)
        {
            /* Copy the scratch buffer to the output */
            Copy32( L_y2, L_t_audio, length_fx );
        }
    ...
    IF( (L_sub(L_bwe_br, HQ_16k40) == 0 || L_sub(L_bwe_br, HQ_13k20) == 0 ) && sub(bwidth_fx, SWB) == 0 )
    {
    ...
    test();
    test();
    IF( (L_sub(L_bwe_br, HQ_16k40) == 0 || L_sub(L_bwe_br, HQ_13k20) == 0) && sub(bwidth_fx, SWB) == 0 && sub(hqswb_clas_fx, HQ_NORMAL) == 0 )
    {
        j = 0;
        move16();
