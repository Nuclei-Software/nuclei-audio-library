...
)
{
    ...
    ELSE IF ( sub(left_rect, 1)==0 && last_core_bfi==ACELP_CORE )
    {
        ...
        ELSE
        {
            ...
            IF( sub(acelp_zir_len, 2*64) >= 0 )
            {
                /* apply a simple low-pass to the ZIR, to avoid potentially unmasked HF content */
                FOR(i=2; i<acelp_zir_len; i++)
                {
                    L_tmp = L_mult(acelp_zir[i-2], 8192);
                    L_tmp = L_mac(L_tmp, acelp_zir[i-1], 11469);
                    acelp_zir[i] = mac_r(L_tmp, acelp_zir[i], 13107);
                    move16();
                }

                L_tmp = L_mult(acelp_zir[acelp_zir_len-1], 13107);
                L_tmp = L_mac(L_tmp, acelp_zir[acelp_zir_len-1], 11469);
                acelp_zir[acelp_zir_len-1] = mac_r(L_tmp, acelp_zir[acelp_zir_len-2], 8192);
                move16();
                L_tmp = L_mult(acelp_zir[acelp_zir_len-2], 13107);
                L_tmp = L_mac(L_tmp, acelp_zir[acelp_zir_len-1], 11469);
                acelp_zir[acelp_zir_len-2] = mac_r(L_tmp, acelp_zir[acelp_zir_len-1], 8192);
                move16();
                FOR(i=acelp_zir_len-3; i>=0; i--)
                {
                    L_tmp = L_mult(acelp_zir[i], 13107);
                    L_tmp = L_mac(L_tmp, acelp_zir[i+1], 11469);
                    acelp_zir[i] = mac_r(L_tmp, acelp_zir[i+2], 8192);
                    move16();
                }
            }
        }
    ...
    IF (gains)
    {
        ...
        IF (sub(k, 4) == 0)
        {
            ...
            FOR (i = 0; i < lg; i += 4)
            {
                ...
                tmp = mac_r(L_mult(pg, 12288/*0.375f Q15*/), g, 20480/*0.625f Q15*/);
                x[i] = L_shl(Mpy_32_16_1(x[i], tmp), e);
                move32();

                tmp = mac_r(L_mult(pg, 4096/*0.125f Q15*/), g, 28672/*0.875f Q15*/);
                ...
                tmp = mac_r(L_mult(g, 28672/*0.875f Q15*/), ng, 4096/*0.125f Q15*/);
                x[i+2] = L_shl(Mpy_32_16_1(x[i+2], tmp), e);
                move32();

                tmp = mac_r(L_mult(g, 20480/*0.625f Q15*/), ng, 12288/*0.375f Q15*/);
        ...
        ELSE IF (sub(k, 5) == 0)
        {
            ...
            FOR (i = 0; i < lg; i += 5)
            {
                ...
                tmp = mac_r(L_mult(pg, 13107/*0.40f Q15*/), g, 19661/*0.60f Q15*/);
                x[i]   = L_shl(Mpy_32_16_1(x[i], tmp), e);
                move32();

                tmp = mac_r(L_mult(pg, 6554/*0.20f Q15*/), g, 26214/*0.80f Q15*/);
                x[i+1] = L_shl(Mpy_32_16_1(x[i+1], tmp), e);
                move32();
                ...
                tmp = mac_r(L_mult(g, 26214/*0.80f Q15*/), ng, 6554/*0.20f Q15*/);
                x[i+3] = L_shl(Mpy_32_16_1(x[i+3], tmp), e);
                move32();

                tmp = mac_r(L_mult(g, 19661/*0.60f Q15*/), ng, 13107/*0.40f Q15*/);
