...
)
{
    ...
    L_tmp = L_shr(enerL, 4);
    test();
    IF(L_sub(L_max(L_WB_fenv1, L_WB_fenv0), L_tmp) > 0  && sub(9856, pitch) > 0)
    {
        ener_var_flag = 1;
        move16();
    }
    ...
    test();
    test();
    IF( sub(shr(enerL_16, 3), tmp) > 0 && L_sub(enerL, L_tmp) > 0 && sub(prev_coder_type,UNVOICED) != 0 && WB_fenv[0] != 0)
    {
        env_var_flag = 1;
        move16();
    ...
    IF( sub(last_extl_fx, WB_BWE) != 0 && sub(tilt_wb_fx, 128) < 0)
    {
...
)
{
    ...
    IF(sub(fenvL_16, shl(SWB_fenv[0], 5)) > 0)
    {
    ...
    ELSE
    {
        ...
        {
            IF(Energy_16 < shr(EnergyL_16, 4) && sub(extl, FB_BWE) == 0)
            {
                FOR(n_band=0; n_band <SWB_FENV; n_band++)
                {
...
)
{
    ...
    FOR( k =0 ; k < nband_lf; ++k )
    {
        ...
        FOR ( i = add((i_mult2(k,blen)),sfidx); i < tmp; ++i )
        {
            IF ( L_sub( L_abs( coeff_out1_fx[i] ), coeff_out1_fx[i] ) != 0 )
            {
                s = -1;
                move16();
