...
)
{
    ...
    IF (sub(sf_flag,1) == 0)
    {
        FOR(i=0; i<stagesm1; i++)
        {
            ...
            Vr_add(x, &Quantizers_fx[CB_lsf_fx[mode]+i][pt_fx], x, N);
        }
    ...
    ELSE
    {
        ...
        {
            pt_fx = i_mult2(indices[i], N);
            Vr_add(x, &Quantizers_p_fx[CB_p_lsf_fx[mode]+i][pt_fx], x, N);
        }
...
)
{
    ...
    /* 0.75 = (1<<add_prec)*T_SIN_PI_2/(2.56f*3200.0f) */
    c = 24576/*0.75f Q15*/;
    move16();
    if (L_sub(int_fs, INT_FS_16k_FX) == 0)
    {
        /* 0.6 = (1<<add_prec)*T_SIN_PI_2/(2.56f*4000.0f) */
        c = 19661/*0.6f Q15*/;
        move16();
    }
    FOR (i = 0; i < m; i++)
    {
        /* lsp[i] = cos(lsf[i] * EVS_PI / (int_fs/2)); */
        L_tmp = L_shr(L_mult(lsf[i],c), add_prec);
        ind = extract_h(L_tmp);
        /* interpolate cosine table at indexes ind and ind+1 */
        ind2 = sub(T_SIN_PI_2, ind);
        lsf_tmp = sincos_t_rad3_fx[s_max(ind2,  negate(ind2))];
        if (ind2 < 0)
        {
            lsf_tmp = negate(lsf_tmp);
        }
        ind2 = sub(T_SIN_PI_2-1, ind);
        offset = sincos_t_rad3_fx[s_max(ind2,  negate(ind2))];
        if (ind2 < 0)
        {
...
)
{
    ...
    IF (sub(safety_net, 1) == 0)
    {
        ...
        IF (sub(nBits, 30) > 0)
        {
            ...
            FOR (i = 0; i < M; i++)
            {
                y_fx[i] = add(y_fx[i], extract_h(L_shl(L_mult0(error_svq_q_fx[i], scale_ARSN_fx[i]), 2)));
                move16();
            }
        }
    }
...
)
{
    ...
    FOR ( i=0; i<stages; i++ )
    {
        ...
        FOR (j=0; j<n; j+=4)
        {
            ...
            depack_4_values(cb[i]+offset+i_mult(3,(shr(j,2))), val0, val1, val2, val3);
