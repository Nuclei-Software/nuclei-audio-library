                     ...
                     )
{
    ...
    ELSE
    {
        ...
        IF (sub(corest->L_frame_fx, L_FRAME16k) == 0)
        {
            push_indice_fx( corest, IND_ACELP_16KHZ, 1, 1 );
        }
        ELSE
        {
            push_indice_fx( corest, IND_ACELP_16KHZ, 0, 1 );
        }
        FOR (i=0; i<stages_37bits; i++)
        {
                                ...
                                )
{
    ...
    */
                   ...
                   st, gen_exc, &Q_new);
    {
        ...
        stcod->lp_ener_fx = L_add(Mult_32_16(stcod->lp_ener_fx, 26214/*0.8f Q15*/), Lener);  /*Q6*/
    }
    ...
    IF ( ( L_sub(stcod->last_core_brate_fx,SID_2k40) > 0 ) && ( sub(stcod->codec_mode,MODE2) == 0 ) )
    {
        ...
        IF ( sub(stcod->last_core_fx,ACELP_CORE) > 0 )
        {
            ...
            left_overlap_mode = stcod->tcx_cfg.tcx_last_overlap_mode;
            move16();
            ...
            tcx_windowing_synthesis_current_frame(  timeDomainOutput,
                                                    ...
                                                    NULL,
                                                    N/2,
                                                    shr(sub(abs_s(stcod->tcx_cfg.tcx_offset), stcod->tcx_cfg.tcx_offset), 1), /* equivalent to: stdec->tcx_cfg.tcx_offset<0?-stdec->tcx_cfg.tcx_offset:0 */
                                                    1,
                                                    0,
                                                    0
                                                 );
