    ...
    *highPassEner_exp = 0;
    ...
    IF (igfBgn > 0)
    {
        ...
        *highPassEner_exp = add(*highPassEner_exp, pPowerSpectrum_exp);
        test();
        test();
        if ( sub(hPrivateData->igfInfo.bitRateIndex, IGF_BITRATE_SWB_9600) != 0 &&
                sub(hPrivateData->igfInfo.bitRateIndex, IGF_BITRATE_RF_SWB_13200) != 0 &&
                sub(hPrivateData->igfInfo.bitRateIndex, IGF_BITRATE_SWB_13200 ) != 0 )
        {
            igfBgn        = shl(igfBgn, 1);
        }
    ...
    IF (!s_or(isTransient, hPrivateData->wasTransient))
    {
        IF (powerSpectrum)
        {
            ...
            FOR (p = 0; p < nT; p++)
            {
                ...
                SFM32 = L_mac0(SFM32,hPrivateData->prevSFM_IIR[p]/*Q13*/,4/*.5f Q3*/);/*15Q16*/
                  ...
                  )
{
    ...
    move16();
    set32_fx(hInstance->spec_be_igf, 0, N_MAX_TCX-IGF_START_MN);
    return;
}
...
)
{
    ...
    FOR (i = 0; i < tmp; i++)
    {
        push_next_indice_fx(st, pBitstream[i], 8);
    }

    bitsLeft = s_and(hPrivateData->igfBitstreamBits, 0x7);
    IF(bitsLeft > 0)
    {
        push_next_indice_fx(st, shr(pBitstream[i], sub(8, bitsLeft)), bitsLeft);
    }
...
void IGFEncApplyMono(const IGF_ENC_INSTANCE_HANDLE                   hInstance,          /**< in:     | instance handle of IGF Encoder                         */
                     ...
                     Word16                                          last_core_acelp     /**< in: Q0  | indicator if last frame was acelp coded                */
                    )
{
