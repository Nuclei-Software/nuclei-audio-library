                      ...
                      )
{
    ...
    IF( (sub(st->frameloop, 60) < 0) && (sub(st->frameloop, 5) > 0) &&(sub(f_tonality_rate[0],9174/* 0.56 Q14 */)<0)
        && (sub(f_tonality_rate[1],8192/* 0.5 Q14 */)<0) && (sub(ltd_stable_rate[1],1966/* 0.06 Q15 */)<0)
        && L_sub(snr, 83886080) < 0)
    {
	IF( sub(st->frameloop, 50) < 0 )
	{
    ...
    ELSE
    {
        ...
        {
            tmpQ = add(3, st->frame_sb_energy_scale);
            FOR(i=0; i<SNR_sb_num; i++)
            {
