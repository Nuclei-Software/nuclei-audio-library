...
)
{
    ...
    FOR( i = 0; i < NHFR; i++ )
    {
        ...
        test();
        if ( Opt_SC_VBR && sub(corX[i][ind1_corX],corX[i][ind_corX]) > 0 )
        {
            ind_corX = ind1_corX;
            move16();
        }
        ...
        IF (Opt_SC_VBR && (sub(mult(pitchX[i][ind], 13107 /*0.4 in Q15*/),pitchX[i][ind_corX]) < 0) &&
            ...
            (sub(corX[i][ind_corX],29491/*0.9 in Q15*/)>=0))
        {
