...
)
{
    ...
    test();
    test();
    IF( (L_sub(st->total_brate_fx, ACELP_9k60) == 0 && sub(st->bwidth_fx, SWB) == 0) ||
        (L_sub(st->total_brate_fx, ACELP_13k20) == 0 && sub(st->bwidth_fx, SWB) == 0) )
    {
        max_low_pre = 0;
        move32();
        FOR (i = 0; i < L_frame; i++)
        {
            ...
            if( L_sub(tmp, max_low_pre) > 0 )
            {
                max_low_pre = L_add(0, tmp);
            }
        }
        ...
        for (i = 0; i < L_spec - L_frame; i++)
        {
            ...
            if( L_sub( tmp, max_high_pre) > 0 )
            {
                max_high_pre = L_add(0, tmp);
            }
        }
    }
        ...
        ( L_sub(st->total_brate_fx, ACELP_13k20) == 0 && sub(st->bwidth_fx, SWB) == 0 ) )
    {
        ...
        FOR (i = 0; i < sf_width; i++)
        {
            ...
            IF( L_sub(tmp, max_low2) > 0 )
            {
                max_low2 = L_add(0, tmp);
                dist_low = i;
                move16();
            }
        }
        ...
        FOR (i = 0; i < sub(L_frame, sf_width); i++)
        {
            ...
            if( L_sub(tmp, max_low1) > 0 )
            {
                max_low1 = L_add(0, tmp);
            }
            if( tmp > max_low2 )
            {
        ...
        {
            Word32 tmp = L_abs(spectrum[L_frame + i]);
            if( L_sub(tmp, max_high) > 0 )
            {
                max_high = L_add(0, tmp);
                dist_high = i;
                move16();
            }
        }
        ...
        test();
        test();
        IF( L_sub(imult3216(max_high, dist_high), imult3216(L_shr(max_low, 2), dist_low)) > 0 && (L_sub(max_low_pre, L_shr(max_high_pre, 4)) > 0) && (L_sub(max_high, L_shl(Mpy_32_16_r(max_low2, max_fac_m), max_fac_s)) > 0) )
        {
            Word16 fac;
            ...
            *-----------------------------------------------------------*/
            ...
            IF ((st->last_core_fx > ACELP_CORE) && (((sub(L_frameTCX, shr(st->L_frameTCX, 1)) == 0) && (st->tcxonly != 0)) || (sub(st->tcx_cfg.tcx_last_overlap_mode, TRANSITION_OVERLAP) == 0)))
            {
                ...
                ELSE
                {
                    ...
                    tcx_windowing_synthesis_past_frame( LPDmem->Txnq
                    ,
                    tcx_cfg->tcx_aldo_window_1_trunc,
                    tcx_cfg->tcx_mdct_window_half,
                    tcx_cfg->tcx_mdct_window_minimum,
                    ...
                    FOR (i=0; i<overlap; i++)
                    {
                        xn_buf16[i] = shl(add(xn_buf16[i], LPDmem->Txnq[i]), TCX_IMDCT_HEADROOM);
                        move16();
                    }
            ...
            IF (tcx_cfg->last_aldo == 0)
            {
                Copy(xn_buf16 + L_frame, LPDmem->Txnq, overlap);
            }
...
)
{
    ...
    ELSE
    {
        ...
        left_overlap_mode = tcx_cfg->tcx_last_overlap_mode;
        move16();
        ...
        right_overlap_mode = tcx_cfg->tcx_curr_overlap_mode;
        move16();
        if (sub(right_overlap_mode, ALDO_WINDOW) == 0)
        {
        ...
        left_overlap_mode, right_overlap_mode,
    ...
    AnalyzePowerSpectrum(st,
                         round_fx(L_shl(Mpy_32_16_1(L_mult0(L_frame,st->L_frame_fx)/*Q0*/,
                                        getInvFrameLen(st->L_frameTCX)/*Q21*/)/*Q6*/,16-6)),
                         L_frame,
                         left_overlap, right_overlap,
                         spectrum, *spectrum_e,
                         pWinMDST,
