...
)
{
    ...
    IF( i_subfr == 0 )
    {
        IF( sub(*tc_subfr,3*L_SUBFR) == 0 )
        {
            ...
            {
                *position = emaximum_fx( Q_new, res_fx + 3*L_SUBFR,s_min(add(T_op_fx[0],2),L_SUBFR), &temp ) + 3*L_SUBFR;
                move16();
                *tc_subfr = 3*L_SUBFR;
                move16();
            }
            ...
            {
                *position = emaximum_fx( Q_new, res_fx + 4*L_SUBFR,s_min(add(T_op_fx[0],2),L_SUBFR), &temp ) + 4*L_SUBFR;
                move16();
                *tc_subfr = 4*L_SUBFR;
                move16();
            }
    ...
    ELSE IF ( sub(*tc_subfr,i_subfr) == 0 )
    {
        ...
        *clip_gain = gp_clip_fx( core_brate, voicing_fx, i_subfr, coder_type, xn_fx, gp_cl_fx, sub(shift_wsp,1) );
    ...
    ELSE IF ( sub(*tc_subfr,i_subfr) < 0)
    {
        IF( sub(L_frame,L_FRAME) == 0 )
        {
            *Jopt_flag = 1;
            ...
            ELSE
            {
                *clip_gain = gp_clip_fx( core_brate, voicing_fx, i_subfr, coder_type, xn_fx, gp_cl_fx, (Q_new+shift-1)  );
        ...
        ELSE  /* L_frame == L_FRAME16k */
        {
            ...
            ELSE
            {
                ...
                *clip_gain = gp_clip_fx( core_brate, voicing_fx, i_subfr, coder_type, xn_fx, gp_cl_fx,Q_new );
...
)
{
    ...
    *gain_pit_fx = corr_xy1_fx( xn_fx, yy1_fx, g_corr_fx, L_SUBFR, 0, NULL);
