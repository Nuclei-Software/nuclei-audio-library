...
)
{
    ...
    FOR ( sb = 0; sb < nBands_search; sb++ )
    {
        nlags_fx[sb] = shl(1, bits_lagIndices_modeNormal_fx[sb]);
        move16();
    }
...
)
{
    ...
    ELSE
    {
        ...
        nBands_fx, sbWidth_fx, lagIndices_fx, fLenLow_fx,
        ...
        FOR(k=0; k<nBands_fx; k++)
        {
            ...
            ELSE
            {
                push_indice_fx( st_fx, IND_LAGINDICES, lagIndices_fx[k], bits_lagIndices_modeNormal_fx[k]);
            }
        }

        convert_lagIndices_pls2smp_fx( lagIndices_fx, nBands_fx, lagIndices_fx,
