...
extern unsigned char enc_input[];
extern size_t enc_input_len;
extern uint8_t enc_output[];
extern size_t enc_output_len;
    ...
    *quietMode = 0;
    ...
    while ( i < argc-4 )
    {
        ...
        if ( strcmp( to_upper(argv[i]), "-MAX_BAND") == 0 )
        {
            ...
            else if ( (*f_bwidth = fopen_notsupported(argv[i+1], "rb")) == NULL )
            {
        ...
        else if ( strcmp( to_upper(argv[i]), "-RF" ) == 0 )
        {
            ...
            if( i < argc - 4 )
            {
                ...
                if ( rf_file_attempt == 1 )
                {
                    ...
                    if ( first_char != '-' )
                    {
                        if ( (*f_rf = fopen_notsupported(rf_file_name, "r")) == NULL )
                        {
    ...
    if ( i < argc-2 )
    {
        ...
        {
            if ( (*f_rate = fopen_notsupported(argv[i], "rb")) == NULL )
            {
        ...
        j = 0;
        while ( j < SIZE_BRATE_TBL && st->total_brate_fx != brate_tbl[j] )
        {
            j++;
        }
    ...
    if( i < argc-2 )
    {
        ...
        if( st->Opt_RF_ON && st->input_Fs_fx == 8000 )
        {
    ...
    if( i < argc-1 )
    {
        if ( (*f_input = memfopen(enc_input, enc_input_len)) == NULL )
        {
    ...
    if( i < argc )
    {
        if ( (*f_stream = memfopen(enc_output, enc_output_len)) == NULL)
        {
    ...
    if ( st->Opt_SC_VBR_fx )
    {
        fprintf(stdout, "Average bitrate:        %s kbps\n", bit_rate_to_string(bit_rate_string, ACELP_5k90));
    }
    else
    {
        fprintf(stdout, "Bitrate:                %s kbps\n", bit_rate_to_string(bit_rate_string, st->total_brate_fx));
    }
    ...
    {
        fprintf(stdout, "\nFB coding not supported below %s kbps. ", bit_rate_to_string(bit_rate_string, ACELP_16k40));
    ...
    {
        fprintf(stdout, "\nSWB coding not supported below %s kbps. Switching to WB.", bit_rate_to_string(bit_rate_string, ACELP_9k60));
    }
