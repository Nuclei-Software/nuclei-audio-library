...
)
{
    ...
    if( st_fx->bitstreamformat == VOIP_RTPDUMP )
    {
        rtpdumpDepackerError = EVS_RTPDUMP_DEPACKER_open(&rtpdumpDepacker, f_stream, st_fx->sdp_hf_only != 0);
        ...
        g192err = G192_NO_ERROR;
    }
    else
    {
        assert( st_fx->bitstreamformat == VOIP_G192_RTP );
    /* open input file */
    ...
    {
        f_offset =  fopen_notsupported( jbmFECoffsetFileName, "w+" );
        if(f_offset == NULL)
        {
    ...
    if (st_fx->bitstreamformat == VOIP_G192_RTP)
    {
    /* read first packet */
    g192err = G192_ReadVoipFrame_compact(g192, au, &auSize,
        ...
        {
            g192err = G192_INVALID_DATA;
        }
        qBit = 1; /* good q_bit for VOIP_G192_RTP */
    }
    else {
        auPtr = au; /* might have been set to RTP packet in prev call */
        rtpdumpDepackerError = EVS_RTPDUMP_DEPACKER_readNextFrame(&rtpdumpDepacker, (uint16_t*)&rtpSequenceNumber, (uint32_t*)&rtpTimeStamp,
                                                                  (uint32_t*)&nextPacketRcvTime_ms,
    ...
    if(g192err != G192_NO_ERROR || rtpdumpDepackerError != EVS_RTPDUMP_DEPACKER_NO_ERROR)
    {
    ...
    for( ; ; )
    {
        ...
        while( nextPacketRcvTime_ms != -1 && nextPacketRcvTime_ms <= systemTime_ms )
        {
            ...
            rxerr = EVS_RX_FeedFrame(hRX, auPtr, auSize, rtpSequenceNumber, rtpTimeStamp,
                                     nextPacketRcvTime_ms, isAMRWB_IOmode, frameTypeIndex, qBit);
          ...
          if (st_fx->bitstreamformat == VOIP_G192_RTP) {
            ...
            if(g192err == G192_EOF
               || rtpdumpDepackerError == EVS_RTPDUMP_DEPACKER_EOF
            )
            {
                /* finished reading */
                nextPacketRcvTime_ms = -1;
                    ...
                    || rtpdumpDepackerError != EVS_RTPDUMP_DEPACKER_NO_ERROR
            )
            {
        ...
        if( nextPacketRcvTime_ms == -1 && ( EVS_RX_IsEmpty(hRX) || nFramesFed < 2 ) )
            break;
        ...
        if ( st_fx->writeFECoffset == 1 && f_offset )
        {
            if ( FEC_hi == 1)
            {
                memfprintf( f_offset, "HI " );
            }
            else
            {
                memfprintf( f_offset, "LO " );
            }
