...
Word16 initFdCngDec (HANDLE_FD_CNG_DEC hs, Word16 scale)
{
    ...
    hs->flag_dtx_mode = 0;
    move16();
    hs->lp_noise  = -167772160l/*-20.f Q23*/;  /* format: Q8.24 */
    hs->lp_speech = 209715200l/* 25.f Q23*/;  /* format: Q8.24 */
    ...
    set32_fx( hs->msMinBuf, 2147483647l/*1.0 Q31*/, MSNUMSUBFR*NPART_SHAPING );

    set32_fx( hs->msCurrentMin, 2147483647l/*1.0 Q31*/, NPART_SHAPING );

    set32_fx( hs->msCurrentMinOut, 2147483647l/*1.0 Q31*/, NPART_SHAPING );

    set32_fx( hs->msCurrentMinSubWindow, 2147483647l/*1.0 Q31*/, NPART_SHAPING );
    ...
    set16_fx( hs->psize_shaping, 0, NPART_SHAPING );
    move16();
    hs->nFFTpart_shaping = 0;
                  ...
                  )
{
    ...
    if(sub(st->hFdCngCom->frame_type_previous,ACTIVE_FRAME)==0)
    {
    ...
    case ACTIVE_FRAME:
        ...
        **************************/
        ...
        test();
        L_tmp = 0;
        FOR( j=st->hFdCngCom->startBand; j<st->hFdCngCom->stopFFTbin; j++ )
        {
            L_tmp = L_add(L_tmp,L_shr(cngNoiseLevel[j],16));
        }
        L_tmp_exp = add(*cngNoiseLevel_exp,16);
        IF (sub(concealWholeFrame,1)==0 && sub(stdec->nbLostCmpt,1)==0 && (L_shl(L_tmp,L_tmp_exp)>21474836 /*0.01f Q31*/))
        {
                            ...
                            )
{
    ...
    IF ( vad == 0 )
    {
        ...
        */
        IF ( totalNoise == 0 )
        {
            logTotalNoise = L_add(-1677721600l/*-0.78125 Q31*/, 0);
        }
        ELSE
        {
            logTotalNoise = BASOP_Util_Log2(totalNoise);
            logTotalNoiseExp = L_shl(L_deposit_l(totalNoise_exp),WORD32_BITS-1-LD_DATA_SCALE);
            logTotalNoise = Mpy_32_16_1(L_add(logTotalNoise,logTotalNoiseExp),24660/*0.75257498916 Q15*/);
        }

        *lp_noise = L_add(Mpy_32_16_1(*lp_noise,32604/*0.995 Q15*/),L_shr(Mpy_32_16_1(logTotalNoise,20972/*0.64 Q15*/),7));
        move32();
    }
    ELSE
    {
        ...
        */
        IF ( Etot == 0 )
        {
            logEtot = L_add(-1677721600l/*-0.78125 Q31*/, 0);
        }
        ELSE
        {
            logEtot = BASOP_Util_Log2(Etot);
            logEtotExp = L_shl(L_deposit_l(Etot_exp),WORD32_BITS-1-LD_DATA_SCALE);
            logEtot = Mpy_32_16_1(L_add(logEtot,logEtotExp),24660/*0.75257498916 Q15*/);
            IF ( sub( frameSize, L_FRAME16k ) == 0 )
            {
                logEtot = L_add( logEtot, -184894985l/*-0.086098436822497 Q31*/ );
            }
            ELSE
            {
                logEtot = L_add( logEtot, -176765584l/*-0.082312889439370 Q31*/ );
            }
        }

        *lp_speech = L_add(Mpy_32_16_1(*lp_speech,32604/*0.995 Q15*/),L_shr(Mpy_32_16_1(logEtot,20972/*0.64 Q15*/),7));
        move32();
    }

    tmp = L_sub(*lp_speech,377487360l/*45.0 Q23*/);
    ...
    *flag_noisy_speech = 0;
    move16();
    if ( L_sub(L_sub(*lp_speech,*lp_noise),234881024l/*28.0 Q23*/) < 0 )
    {
        *flag_noisy_speech = 1;
        move16();
    }
                           ...
                           )
{
    ...
    */
...

    {
        ...
        stdec->lp_ener_fx = L_add(Mult_32_16(stdec->lp_ener_fx, 26214/*0.8f Q15*/), Lener);  /*Q6*/
    }
    ...
    */
    ...
    IF ( st->frame_type_previous == ACTIVE_FRAME && stdec->codec_mode == MODE2 )
    {
        ...
        IF ( stdec->last_core_bfi > ACELP_CORE )
        {
            ...
            left_overlap_mode = stdec->tcx_cfg.tcx_last_overlap_mode;
            move16();
            if (sub(left_overlap_mode, ALDO_WINDOW) == 0)
            {
            ...
            tcx_windowing_synthesis_current_frame(  timeDomainOutput,
                                                    ...
                                                    NULL,
                                                    N/2,
                                                    shr(sub(abs_s(stdec->tcx_cfg.tcx_offset), stdec->tcx_cfg.tcx_offset), 1), /* equivalent to: stdec->tcx_cfg.tcx_offset<0?-stdec->tcx_cfg.tcx_offset:0 */
                                                    1,
                                                    0,
                                                    0
                                                 );
            ...
            {
                FOR (i=0; i<sub(st->frameSize,NS2SA(stdec->sr_core, N_ZERO_MDCT_NS)); i++)
                {
            ...
            ELSE
            {
                ...
                {
                    timeDomainOutput[i] += shl(stdec->syn_Overl[i],TCX_IMDCT_HEADROOM);
                }
            }
        }
        ELSE
    ...
    IF ( scale != 0 )
    {
        ...
        IF ( s1 == 0 )
        {
            FOR (i=0; i < st->stopFFTbin; i++)
            {
                ...
                noise = L_shr(Mpy_32_16_1(L_abs(maskingNoise[i]),25905/*0.79056941504 Q15*/),s2);
        ...
        ELSE
        {
            FOR (i=0; i < st->stopFFTbin; i++)
            {
                ...
                Mpy_32_16_1(maskingNoise[i],25905/*0.79056941504 Q15*/));
                move32();
            }
