...
void open_decoder_LPD( Decoder_State_fx *st, Word32 bit_rate, Word16 bandwidth_mode )
{
    ...
    IF (st->ini_frame_fx == 0)
    {
        ...
        set16_fx(st->syn_Overl, 0, L_FRAME32k/2);

        set16_fx(st->old_synth, 0, OLD_SYNTH_INTERNAL_DEC);
        set16_fx( st->synth_history_fx, 0, Lprot48k + L_FRAME_MAX);
    ...
    ELSE
    {
        ...
        test();
        test();
        IF( sub(st->last_codec_mode,MODE2) == 0 &&
            sub(st->L_frame_fx, st->last_L_frame_fx) != 0 &&
            ( ( sub(st->m_frame_type, SID_FRAME) == 0 && sub(st->last_core_fx, ACELP_CORE) > 0 ) ||
              ( sub(st->last_core_fx, ACELP_CORE) > 0 && sub(st->core_fx, ACELP_CORE) > 0) ||
              st->prev_bfi_fx ) )
        {
            lerp(st->old_out_LB_fx, st->old_out_LB_fx, st->L_frame_fx, st->last_L_frame_fx);
        }
        ...
        IF( sub(st->last_codec_mode,MODE1)==0 && sub(st->last_core_fx,HQ_CORE)==0 )
        {
            ...
            set16_fx(st->syn_Overl, 0, L_FRAME32k/2); /*HQ-CORE(bfi)->TCX don't need it*/
    ...
    ELSE
    {
        ...
        ELSE{
            ...
            move16();
            st->pst_old_syn_fx[NBPSF_PIT_MAX-1]=st->syn[M];
            move16();
        }
    }
    ...
    IF( sub(st->last_codec_mode,MODE1) == 0 || st->ini_frame_fx == 0 )
    {
        ...
        move16();
        st->conceal_eof_gain = 32768/2;/*Q14*/                                        move16();
    ...
    IF (st->prev_bfi_fx)
    {
        ...
        test();
        IF( sub(st->core_fx,ACELP_CORE) == 0 && sub(st->last_core_fx,HQ_CORE) == 0 )
        {
    ...
    st->old_fpitchFB = L_deposit_h(st->pit_min_TCX);
    ...
    st->lp_error_ener = Mpy_32_16_1(L_shl(st->pst_lp_ener_fx, 8), 0x2a86); /* convert from 7Q8 10*log10 -> 15Q16, log2 */
    ...
    st->tcx_cfg.sq_rounding = 12288/*0.375f Q15*/; /*deadzone of 1.25->rounding=1-1.25/2 (No deadzone=0.5)*/    move16();
    ...
    IF(L_sub(st->total_brate_fx, 48000) >= 0 )
    {
        ...
        test();
        test();
        IF (st->ini_frame_fx == 0 || L_sub(st->last_total_brate_fx, HQ_48k) < 0 || sub(st->last_codec_mode, MODE1) == 0 || st->force_lpd_reset )
        {
