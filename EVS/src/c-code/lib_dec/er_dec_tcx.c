...
)
{
    ...
    test();
    IF ( (sub( st->nbLostCmpt, 1 ) == 0) || st->tcxConceal_recalc_exc )
    {
    ...
    IF( sub(st->last_good_fx, UNVOICED_CLAS) > 0  && !(sub(st->last_good_fx, UNVOICED_TRANSITION) == 0 && sub(st->core_ext_mode, GENERIC) == 0) )
    {
        ...
        test();
        test();
        IF( ((sub(st->nbLostCmpt,1) == 0) || st->tcxConceal_recalc_exc) && sub(st->rf_frame_type,RF_TCXFD) >= 0 && sub(st->rf_frame_type,RF_TCXTD2) <= 0 && st->use_partial_copy )
        {
