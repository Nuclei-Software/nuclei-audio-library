...
)
{
    ...
    normalize_spec_fx(4*256, exc_diffQ_fx, mDiff_len, NORMALIZE_SPECS_Q_OUT); /* Factors in Q8 */
    ...
    {
        temp2 = msu_r(-7680*65536, -17564, shl(i,6)) ;   /*-15 in Q9; -0.067 in Q18 and i in Q6= Q9 */
    ...
    Copy(exc_diffQ_fx, dct_out_fx, L_FRAME);
...
)
{
           ...
           (sub(clas, UNVOICED_CLAS) == 0 || sub(clas, VOICED_TRANSITION) == 0)) || sub(coder_type, INACTIVE) == 0 ) )
    {
        ...
        *seed_tcx = extract_l(L_mult0(pitch_buf_fx[0],pitch_buf_fx[3]));
        ...
        test();
        test();
        IF( sub(coder_type, INACTIVE) == 0 && sub(Last_ener_fx, -3*256) > 0 && sub(last_coder_type_fx,UNVOICED) == 0 && rate_switching_reset == 0 ) /* 3.0 x 256 to Go to Q8 */
        {
         ...
         *------------------------------------------------------------*/
        edct_16fx(exc2_fx, dct_exc_in_fx, L_FRAME, 6);
        ...
        edct_16fx(dct_exc_out_fx, exc2_fx, L_FRAME, 6);
        /*------------------------------------------------------------*
