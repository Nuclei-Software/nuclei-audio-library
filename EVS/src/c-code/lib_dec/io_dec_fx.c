...
extern unsigned char dec_input[];
extern size_t dec_input_len;
extern uint8_t dec_output[];
extern size_t dec_output_len;
    ...
    *f_stream = NULL;
    ...
    WHILE ( i < argc-3 )
    {
        ...
        IF ( strcmp( to_upper(argv[i]), "-VOIP") == 0)
        {
            st_fx->Opt_VOIP_fx = 1;
            move16();
            st_fx->bitstreamformat = VOIP_G192_RTP;
            i += 1;
        }
        ELSE IF ( strcmp( to_upper(argv[i]), "-VOIP_HF_ONLY=0") == 0)
        {
            st_fx->Opt_VOIP_fx = 1;
            st_fx->bitstreamformat = VOIP_RTPDUMP;
            st_fx->sdp_hf_only = 0;
            i += 1;
        }
    ...
    if( i < argc - 1 )
    {
        if ( (*f_stream = memfopen(dec_input, dec_input_len)) == NULL)
        {
    ...
    if( i < argc )
    {
        if ( (*f_synth = memfopen(dec_output, dec_output_len)) == NULL )
        {
...
#endif
    fprintf(stdout, "-fec_cfg_file RF   : VoIP mode: Optimal channel aware configuration computed by the JBM\n");
