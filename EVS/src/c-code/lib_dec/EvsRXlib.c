                           ...
                           const char *jbmTraceFileName)
{
    ...
    {
        hEvsRX->jbmTraceFile = fopen_notsupported( jbmTraceFileName, "w" );
        hEvsRX->jbmTraceFile = NULL;
        if( !hEvsRX->jbmTraceFile )
        {
...
)
{
    ...
    if (auSize == 0) {
        return EVS_RX_NO_ERROR; /* ignore empty/NO_DATA frame - shouldn't be transmitted in RTP */
    }
    if ((auSize + 7) / 8 > MAX_AU_SIZE) {
        return EVS_RX_RECEIVER_ERROR;
    }
    ...
    move16();
    dataUnit->isAMRWB_IOmode = isAMRWB_IOmode;
    dataUnit->frameTypeIndex = frameTypeIndex;
    dataUnit->qBit = qBit;
                  ...
                  Word32 systemTimestamp_ms)
{
    ...
    WHILE(sub(pcmdsp_fifo_nReadableSamples(hEvsRX->hFifoAfterTimeScaler), soundCardFrameSize) < 0)
    {
        ...
        IF(dataUnit)
        {
            IF( sub(st->codec_mode,0) != 0 )
            {
                ...
                read_indices_from_djb_fx( st, dataUnit->data, dataUnit->dataSize,
                                          dataUnit->isAMRWB_IOmode, dataUnit->frameTypeIndex, dataUnit->qBit,
                                          tmp, dataUnit->nextCoderType );
        ...
        {
            read_indices_from_djb_fx( st, NULL, 0,
                                      0, 0, 0,
                                      0, 0 );
        }
