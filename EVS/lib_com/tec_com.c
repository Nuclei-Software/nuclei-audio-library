                   ...
                   )
{
    ...
    FOR (slot=startPos; slot < stopPos; slot++)
    {
        ...
        FOR (lb=0; lb < NbTecLowBand; lb++)
        {
            ...
            /* assemble log2(EPS) */
            nrgLog = -668792462l/*-0.31143075889 Q31*/;
            move32();
    ...
    FOR (slot=startPos; slot < stopPos; slot++)
    {
        ...
        FOR (lb=0; lb < NbTecLowBand; lb++)
        {
            ...
            ELSE
            {
                ...
                loBuffer[slot] = -30720/*-0.9375 Q15*/;
                move16();
            }
        }
    }
                                  ...
                                  Word16*  pGainTemp_e  /* Q0 */)
{
    ...
    FOR (slot=startPos; slot < stopPos; slot++)
    {
        ...
        IF ( tmp32 > 0 )
        {
            IF  ( L_sub(tmp32,1006632960l/*0.46875 Q31*/) >= 0 )
            {
                s = add(s,sub(WORD32_BITS,norm_l(BASOP_Util_InvLog2(L_sub(tmp32,1006632960l/*0.46875 Q31*/)))));
                s = add(s,30);
            }
            ELSE
            {
            ...
            logScaleFactor = L_shl(L_mult0(512/*1.0/(1<<LD_DATA_SCALE) Q15*/,s),16);
    ...
    IF (code > 0)
    {
        IF (sub(code, 2) != 0)
        {
            calcLoTempEnv_TBE_Fx(loBuffer_Fx + MAX_TEC_SMOOTHING_DEG, noCols,
                                 loTempEnv_Fx, 19531/*0.5f * ratioHiLoFacDec Q15*/);
        }
        ELSE
        {
                        ...
                        )
{
    ...
    FOR (i=i_offset; i<N_TEC_TFA_SUBFR; i++)
    {
        ...
        {
            /* 1.0e+12 */
            inv_curr_enr_m[i] = 28282/*0.8631 Q15*/;
            move16();
            inv_curr_enr_e[i] = 39;
            move16();
        }
    ...
    move16();
    lower_limit_gain_m = 26214/*0.1 * 8 Q15*/; /* norm = 0 */move16();
    ...
    /* upper_limit_gain */
    upper_limit_gain_m = 19661/*0.6f Q15*/; /* norm = 0 */ move16();
    upper_limit_gain_e = 1;
    move16();
    IF (sub(code,  LOBUF_NO_SMOOTHING_MODE) == 0)
    {
        upper_limit_gain_m = 24576/*0.75f Q15*/; /* norm = 0 */ move16();
...
)
{
    ...
    calcLoTempEnv_Fx(loBuffer_Fix+MAX_TEC_SMOOTHING_DEG,noCols,loTempEnv_Fix,26041/*0.5 * ratioHiLoFac Q15*/);
    ...
    *corrFlag = 0;
    ...
    ELSE
    {
        ...
        EQ1 = L_sub(L_shl(corrCoef_Fix, 8),28819/*thCorrCoef Q15*/);
        EQ2 = L_sub(L_shl(hiVar_Fix,0),Mpy_32_16_1(loVar_Fix,11957/*thRatio Q15*/));
        EQ3 = L_sub(L_shr(hiVar_Fix,2),Mpy_32_16_1(loVar_Fix,16620/*thRatio2 Q13*/));
...
void tecEnc_TBE_fx(Word16* corrFlag, const Word16* voicing, Word16 coder_type)
{
    ...
    IF( sub(*corrFlag, 1) == 0 )
    {
        ...
        if( sub(coder_type,INACTIVE) == 0 || ((sub(voice_sum, 11469/*0.35 Q15*/) > 0 && sub(voice_sum, 18022/*0.55 Q15*/) < 0) && (sub(voice_diff, 6554/*0.2 Q15*/) < 0)) )
        {
            *corrFlag = 0;
            move16();
        }
    }
    if( sub(voice_sum, 19661/*0.6 Q15*/) > 0 ) /*if( voice_sum > 0.6 * 2 )*/
    {
        *corrFlag = 0;
        move16();
    }
