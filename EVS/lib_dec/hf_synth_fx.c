...
)
{
    ...
    *memExp1 = exp1;
    ...
    Syn_filt_s( 0, Ap, M, HF_exc, HF_syn, L_SUBFR16k, mem_syn_hf, 1 );
...
void hf_synth_amr_wb_init_fx(
    ...
    Word16 mem_syn_hf_fx[],            /* HF LPC synthesis filter initialization                */
...
)
{
    ...
    shift = sub(qhf, Q_out);
    Syn_filt_s( shift, Ap, M, exc16k, HF_syn, L_SUBFR16k, mem_syn_hf, 1 ); /*Q_out=qhf-shift */
