...
)
{
    ...
    ELSE
    {
        ...
        FOR (i = 0; i < st->nb_subfr; i ++)
        {
            pitch_buf[i] = L_deposit_h(st->pit_max); /*15Q16*/
            pgainT[i] = 0;
            move16();
            pT[i] = L_SUBFR;
            move16();
    ...
    IF ( sub(st->last_good_fx , UNVOICED_TRANSITION)>=0 )
    {
        ...
        tilt_code = mac_r(214748368l/*0.1f Q31*/, 3277/*0.1f Q15*/ , st->voice_fac);

        gain_inov = mult_r(gain_inov, sub(32767/*1.0f Q15*/, tilt_code)); /* Q15 * 2^s_gain_inov */
    }
    ...
    IF(st->last_good_fx >= UNVOICED_TRANSITION)
    {
        ...
        FOR (i_subfr = 0; i_subfr < st->L_frame_fx; i_subfr += L_SUBFR)
        {
            ...
            set16_fx(mem, 0, M);
            h1[0] = 1024/*1.0f/((float)(1 << scale_h1)) Q15*/;
            move16();
    ...
    *Qf_mem_syn = Qf_syn;
    ...
    n = extract_h(L_mult(st->L_frame_fx,9216/*(float)N_ZERO_MDCT_NS/(float)FRAME_SIZE_NS Q15*/));

    st->Q_old_wtda_LB = getScaleFactor16(syn+st->L_frame_fx-n,
                                         sub(st->L_frame_fx,n));
    move16();
