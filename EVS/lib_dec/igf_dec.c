                                 ...
                                 )
{
    ...
    g = getSqrtWord32(L_mult(divide3232(totalNoiseNrg, rE), 8192/*1.0f / 4.0f Q15*/));
    g = shl(g, 1);
                    ...
                    )
{
    ...
    FOR (tile_idx = 0; tile_idx < nTiles; tile_idx++)
    {
        ...
        IF (sub(IGF_WHITENING_STRONG, hPrivateData->currWhiteningLevel[tile_idx]) == 0)
        {
            ...
            {
                abs_sum = L_add(abs_sum, L_abs(src_spec[i]));
            }
                    ...
                    )
{
    ...
    /* reset virtual spec */
    set16_fx(flag_sparse, 0, N_MAX_TCX-IGF_START_MN);
    set32_fx(virtualSpec, 0, N_MAX_TCX-IGF_START_MN);
    *virtualSpec_e = *spectrum_e;
    ...
    dE    = mult_r(dE, 26755/*0.81649658092772603273242802490196f Q15*/);       /* 0.81649658092772603273242802490196f = sqrt(1/1.5)) */
    dE_e  = shift;
    move16();
    ...
    IF (hGrid->infoIsRefined != 0)
    {
        FOR (sfb = start_sfb; sfb < stop_sfb; sfb += 2)
        {
            ...
            gn   = hPrivateData->igf_curr[shr(sfb,1)];
            move16();
            move16();
                                    ...
                                    )
{
    ...
    Word16 div;
    Word16 nrm_i;
    Word16 nrm_tab[] = {2341 /* 1/14 */, 2521 /* 1/13 */, 2731 /* 1/12 */, 2979 /* 1/11 */, 3277 /* 1/10 */, 3641 /* 1/9 */, 4096 /* 1/8 */, 4681 /* 1/7 */};
    ...
    FOR (j = start; j < stop - level; j++)
    {
        ...
        div    = shl(1, tmp_16);
        out[j] = Mpy_32_16_1(L_shl(in[j], s_l), div);
        move32();
                    ...
                    )
{
    ...
    set16_fx(hInstance->infoTCXNoise, 0, IGF_START_MX);
                  ...
                  )
{
    ...
    IF (IGFCommonFuncsIGFConfiguration(bitRate, mode, &hPrivateData->igfInfo, rf_mode) != 0)
    {
        ...
        test();
        IF ((sub(hPrivateData->igfInfo.bitRateIndex, IGF_BITRATE_SWB_48000) <= 0) || (sub(hPrivateData->igfInfo.bitRateIndex, IGF_BITRATE_FB_48000) <= 0))
        {
