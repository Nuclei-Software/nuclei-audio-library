...
)
{
    ...
    ELSE
    {
        ...
        test();
        IF ( st->igf != 0 && (sub(st->core_fx, ACELP_CORE) == 0) )
        {
    ...
    test();
    test();
    IF( ( st->core_fx != ACELP_CORE || st->tcx_cfg.lfacNext > 0 ) && st->use_partial_copy == 0 )
    {
    ...
    IF( st->use_partial_copy == 0 )
    {
        ...
        test();
        IF ((sub(st->last_core_fx, ACELP_CORE) == 0) || (sub(st->last_core_fx, AMR_WB_CORE) == 0))
        {
        ...
        {
            test();
            IF ((sub(st->core_fx, TCX_10_CORE) == 0) && (sub(st->tcx_cfg.tcx_curr_overlap_mode, ALDO_WINDOW) == 0))
            {
            ...
            ELSE
            {
                ...
                move16();
                test();
                if ((sub(st->core_fx, TCX_10_CORE) != 0) && (sub(st->tcx_cfg.tcx_curr_overlap_mode, FULL_OVERLAP) == 0))
                {
        ...
        IF (sub(st->core_fx, ACELP_CORE) != 0)
        {
            ...
            test();
            test();
            if ((sub(st->core_fx, TCX_20_CORE) == 0) && (tmp == 0) && (sub(st->last_core_fx, ACELP_CORE) != 0) && (sub(st->last_core_fx, AMR_WB_CORE) != 0))
            {
                st->tcx_cfg.tcx_curr_overlap_mode = ALDO_WINDOW;
                move16();
            }
    ...
    IF( st->use_partial_copy == 0 )
    {
        ...
        test();
        IF (st->enableTcxLpc && sub(st->core_fx, ACELP_CORE) != 0)
        {
            Word16 tcx_lpc_cdk;
        ...
        ELSE
        {
            ...
            ELSE IF (sub(st->lpcQuantization, 1) == 0)
            {
                test();
                test();
                IF(L_sub(st->sr_core, 16000) == 0 && sub(*coder_type, VOICED) == 0 && sub(st->core_fx, ACELP_CORE) == 0)
                {
    ...
    test();
    test();
    IF( sub(st->core_fx, ACELP_CORE) == 0 && st->use_partial_copy == 0 )
    {
        /* Target Bits */
     ...
     *--------------------------------------------------------------------------------*/
    test();
    IF( sub(st->core_fx, TCX_20_CORE) == 0 && st->use_partial_copy == 0 )
    {
        flag_ctx_hm = 0;
        move16();
        ...
        IF (sub(st->last_core_bs_fx, ACELP_CORE) == 0)
        {
            /* ACE->TCX transition */
        ...
        test();
        IF (st->tcx_lpc_shaped_ari != 0 && sub(st->last_core_bs_fx, ACELP_CORE) != 0)
        {
        ...
        /*Context HM flag*/
        test();
        IF ( st->tcx_cfg.ctx_hm && sub(st->last_core_bs_fx, ACELP_CORE) != 0 )
        {
            prm[j] = get_next_indice_fx(st, 1);
            move16();
    ...
    IF ( sub(st->core_fx, TCX_10_CORE) == 0)
    {
        ...
        FOR (k = 0; k < 2; k++)
        {
            ...
            IF (st->tcx_cfg.fIsTNSAllowed)
            {
                IF( sub(st->last_core_bs_fx, ACELP_CORE) == 0 && (k == 0))
                {
                    st->BER_detect = 1;
    ...
    IF(!st->use_partial_copy)
    {
        IF (sub(sub(*total_nbbits, bitsRead[0]), sub(st->next_bit_pos_fx, start_bit_pos)) < 0)
        {
            st->BER_detect = 1;
            move16();
