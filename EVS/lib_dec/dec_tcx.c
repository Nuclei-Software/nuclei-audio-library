...
)
{
    ...
    IF (bfi != 0)
    {
        ...
        IF (left_rect != 0)
        {
            ...
            L_spec_con = add(L_spec_con, shr(st->tcx_cfg.tcx_coded_lines, 2));
        }
    }
    ELSE
    {
        ...
        IF ( frame_cnt == 0 && sub(st->last_core_bs_fx, ACELP_CORE) == 0 )
        {
    ...
    IF (bfi == 0)
    {
        ...
        test();
        test();
        IF(tcx_cfg->ctx_hm != 0 && ( (st->last_core_bs_fx != ACELP_CORE) || (frame_cnt > 0) ) )
        {
