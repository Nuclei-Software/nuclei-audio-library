...
)
{
    ...
    IF( !Opt_AMR_WB )
    {
        ...
        {
            test();
            if( sub(L_subfr,L_FRAME/2) == 0 && i_subfr != 0 )
            {
                pit_flag = L_SUBFR;
                move16();
            }
            ...
            test();
            test();
            IF( sub(L_subfr,L_FRAME/2) == 0 && i_subfr != 0 && sub(pitch_index,32) >= 0 )  /* safety check in case of bit errors */
            {
...
)
{
    ...
    test();
    test();
    IF( sub(add((*T0<<2),*T0_frac),add((PIT_MAX<<2),2)) > 0 && pit_flag == 0 && !Opt_AMR_WB )
    {
        *T0 = L_SUBFR;
        move16();
        *T0_frac = 0;
...
)
{
    ...
    /* biterror detection mechanism */
    test();
    IF( sub(add((*T0<<2),*T0_frac),(PIT16k_MAX<<2)) > 0 && sub(nBits,9) >= 0 )
    {
        *T0 = L_SUBFR;
        move16();
        *T0_frac = 0;
