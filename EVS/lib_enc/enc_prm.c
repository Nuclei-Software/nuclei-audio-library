...
)
{
    ...
    ELSE
    {
        IF ( core==ACELP_CORE )
        {
            ...
            IF( sub(st->rf_mode,1) == 0)
            {
                ...
                WHILE( L_sub(acelp_sig_tbl[idx], tmp32) != 0 )
                {
                    idx = add(idx,1);
                }
            ...
            ELSE
            {
                ...
                ELSE
                {
                    ...
                    IF( sub(st->rf_mode,1) == 0)
                    {
                        ...
                        idx = add(idx,1);
                        start_idx = idx;
                        tmp32 = SIG2IND_fx(st->tcx_cfg.coder_type, st->bwidth_fx, st->sharpFlag, st->rf_mode);
                        WHILE( L_sub(acelp_sig_tbl[idx], tmp32)!= 0 )
                        {
                            idx = add(idx,1);
                        }
    ...
    IF (!(core==ACELP_CORE&&st->tcx_cfg.lfacNext<=0))
    {
        ...
        move16();
        test();
        IF( sub(last_core, ACELP_CORE) != 0 || sub(core, TCX_10_CORE) == 0 )
    ...
    IF ( s_or(sub(core,TCX_20_CORE) == 0, sub(core,HQ_CORE) == 0) )
    {
        ...
        {
            push_next_indice_fx(st, st->Tonal_SideInfo, 1);
        }
        ...
        ELSE
        {
            ...
            lgFB,
            prm[j-1], /* lastnz */
            nbits_tcx, NPRM_RESQ * st->tcx_cfg.resq, phm_cfg);
        }
