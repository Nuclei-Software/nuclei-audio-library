...
)
{
    ...
    test();
    test();
    IF( sub(L_frame,last_L_frame) != 0 && L_sub(total_brate,ACELP_24k40) == 0 && L_sub(i_subfr, 5*L_SUBFR) < 0 )
    {
...
)
{
    ...
    IF(sub(mode,NORMAL_OPERATION)==0 || (sub(mode,FULL_BAND)==0))
    {
        ...
        IF(use_prev_sf_pit_gain == 0)
        {
            ...
            if (clip_gain && sub(gain1,15565/*0.95 Q14*/) > 0)
            {
    ...
    IF(sub(mode,NORMAL_OPERATION)==0 || sub(mode,LOW_PASS)==0)
    {
        ...
        /* clip gain if necessary to avoid problem at decoder */  test();
        if (clip_gain && sub(gain2,15565/*0.95 Q14*/) > 0)
        {
...
)
{
    ...
    cb_shape_fx(pre_emphasis, pitch_sharpening, phase_scrambling, formant_enh, formant_tilt,
                formant_enh_num, formant_enh_den, Aq, h2, tilt_code, pitch, 1);
    ...
    *pt_indice += 8;
    ...
    cb_shape_fx(pre_emphasis, pitch_sharpening, phase_scrambling, formant_enh, formant_tilt,
                formant_enh_num, formant_enh_den, Aq, code, tilt_code, pitch, 1);
