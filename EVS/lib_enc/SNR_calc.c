...
)
{
    ...
    *lf_snr_smooth =  L_shr(*lf_snr_smooth, sub(q_divout1, 25));
    ...
    {
        l_snr = L_add(0, 161061275/* 4.8 Q25 */);
    }
                ...
                )
{
    ...
    if(sub(tmp_lt_noise_sp_center, 818/* 0.8 Q10 */) > 0)
    {
    ...
    *lt_snr_org_fp = lt_snr;
    ...
    IF(sub(bg_energy_count, 56)<0||sub(fg_energy_count,56)<0)
    {
        lt_snr = L_add(0, 70464302/* 2.1 Q25 */);
    }
    ...
    if(L_sub(lt_snr,67108862/* 2.0 Q25 */)>0)
    {
        lt_snr = L_add(0, 67108862/* 2.0 Q25 */);
    }
...
)
{
    ...
    test();
    IF( (L_sub(L_shr(tsnr,1) , 43620759/* 2.6f/2.0f Q25 */)<0 )&&tsnr>0)
    {
        pre_snr[0] = tsnr;
        move32();
    }
             ...
             )
{
    ...
    FOR(i=0; i<SNR_sb_num; i++)
    {
        ...
        if(L_sub(tmp, -3355443/* -0.10 Q25 */)>0)
        {
            snr_tmpidx[i] = tmp;
            move32();
        }
                           ...
                           )
{
    ...
    IF(sub(bw_index, CLDFBVAD_SWB_ID)== 0)
    {
        ...
        IF(sub(sp_center[3], 2864/* 2.80 Q10 */)>0)
        {
            snr_delta = L_add(snr_delta,0);
        }
        ELSE IF(sub(sp_center[2], 2660/* 2.6 Q10 */)>0)
        {
            snr_delta = L_add(snr_delta, 1006633/* 0.03 Q25 */);
        }
        ELSE IF(sub(sp_center[2], 1637/* 1.6 Q10 */)>0)
        {
            snr_delta = L_add(snr_delta ,1677722/* 0.05 Q25 */);
        }
        ELSE IF(sub(sp_center[3], 1432/* 1.4 Q10 */)>0)
        {
    ...
    ELSE IF(sub(bw_index, CLDFBVAD_NB_ID)== 0)
    {
        ...
        IF(sub(sp_center[3], 3069/* 3.0 Q10 */)>0)
        {
            snr_delta = L_add(snr_delta,0);
        }
        ELSE IF(sub(sp_center[2], 2660/* 2.6 Q10 */)>0)
        ...
        ELSE IF(sub(sp_center[2], 1494/* 1.46 Q10 */)>0)
        {
            snr_delta = L_add(snr_delta , 3355443/* 0.10 Q25 */);
        }
        ELSE
        ...
        test();
        test();
        IF(L_sub(continuous_speech_num, 80) > 0 && L_sub(fg_energy_est_start, 1) == 0 && (sub(sp_center[0],1432/* 1.4 Q10 */)>0))
        {
    ...
    ELSE
    {
        snr_delta = L_add(33554431/* 1.0 Q25 */,0);
    }
