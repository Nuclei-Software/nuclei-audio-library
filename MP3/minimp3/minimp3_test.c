...
#define ASSERT(c) if (!(c)) { printf("failed, line=%d\n", __LINE__); exit(1); }
    ...
    {
        /* Regression: out-of-range sample seek must not corrupt to_skip and crash in mp3dec_ex_read(). */
        static const uint8_t seek_oob_buf[] = {
            0xff,0xfd,0x16,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
            0xa5,0xce,0xa7,0x8b,0xff,0xff,0x00,0x00,0x00,0x00,
            0x47,0x45,0x58,0x00,0xff,0xfd,0x18,0xd6,0x64,0x41,
            0xae,0xce,0xd6,0x97,0x8a,0x4e,0x18,0x8b,0x82,0xff,
            0xfd,0x18,0xd6,0x45,0x54,0x18,0x8b,0x82,0x47,0x45,
            0x58,0x00,0xff,0xfd,0x18,0xd6,0x64,0x41,0x50,0x45,
            0x54,0x41,0x01,0x18,0xd6,0x64,0x41,0x50,0x45,0x54,
            0x41,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x23,0xff,
            0xfd,0x18,0xd6,0x64,0x41,0x50,0x45,0x54,0x41,0x47,
            0x45,0x58,0x00,0xff,0xfd,0x18,0xd6
        };
        mp3d_sample_t pcm[MINIMP3_MAX_SAMPLES_PER_FRAME];
        size_t r, total = 0;
        ret = mp3dec_ex_open_buf(&dec, seek_oob_buf, sizeof(seek_oob_buf), MP3D_SEEK_TO_SAMPLE | MP3D_ALLOW_MONO_STEREO_TRANSITION);
        ASSERT(0 == ret);
        do
        {
            r = mp3dec_ex_read(&dec, pcm, MINIMP3_MAX_SAMPLES_PER_FRAME);
            total += r;
        } while (r);
        ASSERT(2304 == total);
        ret = mp3dec_ex_seek(&dec, 3888454656ULL);
        ASSERT(0 == ret);
        r = mp3dec_ex_read(&dec, pcm, MINIMP3_MAX_SAMPLES_PER_FRAME);
        ASSERT(0 == r);
        mp3dec_ex_close(&dec);
...
static const char *g_files[] = {
    ...
    "vectors/l3-nonstandard-vbrtag-oob-read.bit",
    "vectors/l3-si.bit",
